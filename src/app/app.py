...
async def retrieve_user_handler(session: Dict[str, Any], connection: ASGIConnection[Any, Any, Any, Any]) -> User | None:
    print("retrieve_user_handler called")    
    ...
    try:
        ...
        print(f"Invalid user ID in session: {user_id_str}")
        return None

    sa_plugin: SQLAlchemyPlugin = connection.app.state.sqlalchemy_plugin
    async with sa_plugin.db_session() as db_session:
        try:
            return await get_user_by_id(user_id, db_session)
...
app = Litestar(
    route_handlers=[login_page, login_redirect, login, panel_page, check_username, create_new_user, 
    ...
    template_config=TemplateConfig(
        directory=Path("static/templates"),
